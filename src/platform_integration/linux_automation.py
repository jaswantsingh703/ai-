...
class LinuxAutomation:
    ...
    def get_brightness(self):
        ...
        """
        try:
            result = subprocess.run("xrandr --verbose | grep -i brightness", shell=True, capture_output=True, text=True)
    ...
    def set_brightness(self, level):
        ...
        """
        try:
            os.system(f"xrandr --output $(xrandr | grep ' connected' | cut -d' ' -f1) --brightness {level}")
...
if __name__ == "__main__":
    linux_auto = LinuxAutomation()
    linux_auto.open_application("firefox")
    linux_auto.execute_command("ls -l")
    # linux_auto.shutdown() # Be careful when uncommenting
    # linux_auto.restart()
